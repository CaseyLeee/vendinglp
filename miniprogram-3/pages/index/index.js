...
Page({
  ...
  onSubmit(e){
    ...
    this.setData({ goodschoose: e.currentTarget.dataset.item });
    let  that=this;
    wx.login({
      success (res) {
        if (res.code) {
    ...
    wx.request({
      ...
      success(res) {
        ...
        if(res.data.code==1){
          ...
          that.setData({list:res.data.data.containList.filter((item)=>{
            return item.number!=8&& item.number!=7
          })})
          that.setData({listfix:res.data.data.containList.filter((item)=>{
            return item.number==8|| item.number==7
          })})
